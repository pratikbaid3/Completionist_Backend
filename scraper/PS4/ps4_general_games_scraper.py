...
c = conn.cursor()
c.execute(
    """CREATE TABLE IF NOT EXISTS PS4_Games
    (game_name TEXT,game_image_link TEXT,game_guide_url TEXT,GOLD TEXT,SILVER TEXT, BRONZE TEXT,PLATINUM TEXT)"""
)
...
def adding_data_to_db(game_name, game_image_link, game_guide_url):
    try:
        c.execute(
            """INSERT INTO PS4_Games (game_name,game_image_link,game_guide_url,GOLD,SILVER,BRONZE,PLATINUM) VALUES ("{}","{}","{}","{}","{}","{}","{}");""".format(
                game_name, game_image_link, game_guide_url, "0", "0", "0", "0"
            )
        )
