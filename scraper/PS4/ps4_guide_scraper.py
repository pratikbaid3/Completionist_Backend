...
import sqlite3
import get_youtube_link_from_guide as youTubeLinkExtractor
...
try:
    ...
    c = conn.cursor()
    c.execute(
        """CREATE TABLE IF NOT EXISTS PS4_Games_Guide
        (game_name TEXT,trophy_name TEXT,trophy_image TEXT,trophy_type TEXT,trophy_description TEXT,trophy_guide TEXT,youtube_link TEXT)"""
    )
    ...
    def adding_data_to_db(
        ...
        try:
            c.execute(
                """INSERT INTO PS4_Games_Guide (game_name,trophy_name,trophy_image,trophy_type,trophy_description,trophy_guide,youtube_link) VALUES ("{}","{}","{}","{}","{}","{}","{}");""".format(
                    str(game_name),
                    str(trophy_name),
                    str(trophy_image),
                    str(trophy_type),
                    str(trophy_description),
                    trophy_guide,
                    youtube_link,
                )
            )
    ...
    for i in range(1510, 2201):
        ...
        trophy_name_list = [
            str(trophy).replace('"', "'") for trophy in trophy_name_list
        ]
        # Trophy Description
        trophy_description_list = page_soup.find_all("div", {"class": "achilist__data"})
        trophy_description_list = [
            description.find("p", recursive=False).text
            for description in trophy_description_list
        ]
        trophy_description_list = [
            str(trophy).replace('"', "'") for trophy in trophy_description_list
        ]
        # Trophy Type
        ...
        trophy_guide_list = [
            trophy.find("div", recursive=False) for trophy in trophy_guide_list
        ]
        trophy_guide_list = [
            str(trophy).replace('src="//', 'src="https://')
            for trophy in trophy_guide_list
        ]
        trophy_guide_list = [
            str(trophy).replace('"', "'") for trophy in trophy_guide_list
        ]
        youtube_links = [
            youTubeLinkExtractor.getYoutubeLink(trophy) for trophy in trophy_guide_list
        ]
        trophy_guide_list = [
            youTubeLinkExtractor.removeIframe(trophy) for trophy in trophy_guide_list
        ]
